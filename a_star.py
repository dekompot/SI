...
class Dijkstra():
    ...
    def run(self, a_start: Stop, b_end: Stop, start_time: str, clear_logs: bool= False):
        ...
        self._proceed(a_start, b_end, start_time, Dijkstra.euclidean)
    ...
    def manhattan(next_node: Stop, end_node: Stop) -> float:
        return abs(next_node.longitude - end_node.longitude) + abs(next_node.latitude - end_node.latitude)
    
    def euclidean(next_node: Stop, end_node: Stop) -> float:
        return ((next_node.longitude - end_node.longitude)**2 + (next_node.latitude - end_node.latitude)**2)**.5
